...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    because:
      "DESIGN.md: mono \"is the label beside the work, never headings or body copy\", and the Serif Speaks rule gives headings to Fraunces. 26 h2/h3 elements were set in `label-mono` — 11.52px uppercase #606976, quieter and smaller than the body they introduced, and on /colophon pixel-identical to the field labels nested inside them. An inverted hierarchy on the three pages that explain the work. DESIGN.md:325's \"any new section should introduce itself with one\" means a label ABOVE the heading, not instead of it.",
...
const ALLOWED: Array<{ file: string; rule: string; reason: string }> = [
  ...
  },
  {
    ...
    reason:
      "DESIGN.md:322 specifies the footer's column headings in wall-label mono, and the Wall Label Rule agrees: a column name is metadata, not a section heading. The only heading on the site the label voice is documented to own.",
  },
  {
    file: "src/components/home/hero-ask.tsx",
    rule: "focus-opt-out",
    reason:
      "DESIGN.md:315 specifies the underline field's focus outright: \"Focus turns the rule Forest Ink with no ring.\" It is the one control the document gives its own treatment. Worth revisiting — a 1px border colour change is the weakest indicator on the site, and WCAG 2.4.11 wants more area than 672px of hairline — but that is a change to the design system, not drift from it.",
  },
  {
