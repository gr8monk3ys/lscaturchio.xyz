...
Scripts live in `package.json`. Examples use `npm`, but Bun is also supported
and is what CI runs.

## Architecture Notes

- `public/my-data`: retrieval source files used by embeddings and some content integrations; the `blog-*.md` files are generated by `npm run sync-retrieval-corpus` — do not hand-edit
- `src/generated/audio-manifest.ts`: generated from `public/audio` — do not hand-edit
...
- When a blog post changes, run `npm run sync-retrieval-corpus`; rerun embeddings if chat relevance matters
