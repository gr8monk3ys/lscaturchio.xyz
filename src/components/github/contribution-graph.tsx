...
export function ContributionGraph() {
  const [data, setData] = useState<{ totalContributions: number; weeks: ContributionWeek[] } | null>(null)
        ...
        <div className="inline-block min-w-full">
          ...
          <div className="flex gap-1 mb-2 ml-8">
            {displayMonths.map((month, i) => (
              <div key={i} className="text-xs text-muted-foreground" style={{ width: '52px' }}>
                {i % 2 === 0 ? month : ''}
              </div>
            ))}
          </div>
